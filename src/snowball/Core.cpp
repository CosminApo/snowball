					...
					for (size_t eq = 0; eq < entities.size(); eq++) // Loop through all the entities.
					{
						if (et != eq) // If its not testing against itself.
						{
							if (entities.at(eq)->getCollider()) // If the other entity has a collider.
							{
								...
								{
									if (checkXCollision(entities.at(et), entities.at(eq))) // Check for collision in the X axis.
									{
										if (checkYCollision(entities.at(et), entities.at(eq))) // Check for collision in the Y axis.
										{
											if (checkZCollision(entities.at(et), entities.at(eq))) // Check for collision in the Z axis.
											{
